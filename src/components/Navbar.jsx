...
const Navbar = () => {
	...
	return (
		<nav id='main-nav' className='fixed w-full h-[70px] px-12 bg-primary-600 text-custom_gray-500' aria-label='main nav'>
			<div className='h-full flex justify-between items-center mx-auto max-w-[1000px]'>
				<div className='z-10' tabIndex="-1">
					<Link to='home' spy={true} smooth={true} duration={250}>
						<img className='svg-color-accent cursor-pointer' src={Logo} alt='website logo' style={{ width: '50px' }} />
					</Link>
				</div>
				...
				<ul id="main-menu" className='hidden md:flex' role="menubar">
					{navLinks.map(item => (
						<li key={item.name} role="presentation">
							<Link activeClass='font-bold text-accent' to={item.link} spy={true} smooth={true} duration={500} role="menuitem">
								{item.name}
							</Link>
						</li>
					))}
				...
				{/* burger */}
				<button onClick={handleClick} className='md:hidden z-10 p-4 pr-0' aria-label='toggle mobile main menu' aria-expanded={nav} aria-controls='mobile-menu'>
					{!nav ? <FaBars /> : <FaTimes />}
				</button>
			...
			<div className='hidden lg:flex fixed flex-col top-[35%] left-0'>
				<ul id='socials-menu' role='menu' aria-label='socials menu'>
					...
					<li className='w-[160px] h-[60px] flex justify-between items-center ml-[-100px] hover:ml-[0px] shadow-sm hover:shadow-lg duration-300 bg-accent-500'
						role='presentation'>
						<Link
							onClick={handleClick}
							activeClass='font-bold'
							...
							smooth={true}
							duration={500}
							className='flex justify-between items-center w-full text-custom_gray-500'
							href='/'
							role='menuitem'>

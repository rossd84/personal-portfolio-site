...
const Section = ({
  ...
  return (
    <section id={id && id} name={sectionName} className={classList} style={style} aria-labelledby={ariaLabelledBy}>
          ...
          <div className="max-w-[1000px] w-full mb-8 flex justify-center md:justify-start flex-col">
            <div className="flex">
              <h2
                ...
                className="text-3xl md:text-4xl text-custom_gray-500 font-bold border-b-4 border-accent-500 tracking-wider"
              >
                {header}
              </h2>
            </div>
            {subheader && <p className="py-4">{subheader}</p>}
          </div>
        )}
        {children}
      </div>
